...
class Promotion extends BaseService implements IPromotion
{
    ...
    public function addPromotionMansong($mansong_name, $start_time, $end_time, $shop_id, $remark, $type, $range_type, $rule, $goods_id_array)
    {
        ...
        try {
            $err = 0;
            $count_quan = $goods_mansong->getQuanmansong($start_time, $end_time,$shop_id);
			...
			$examine_status=0;$status=0;
			if(empty($operate_config->is_mansong_toExamine)){
                $examine_status=1;$status=1;
            }
            $data = array(
    ...
    public function addPromotiondiscount($discount_name, $start_time, $end_time, $remark, $goods_id_array)
    {
        ...
        try {
			...
			if(empty($operate_config->is_discount_toExamine)){
                $examine_status=1;$status=1;  //1为审核通过，即无需审核
            } else {$examine_status=0;$status=0;}
            $data = array(
    ...
    public function closePromotionDiscount($discount_id)
    {
        ...
        try {
            $retval = $promotion_discount->save([
                'status' => 3,'modify_time' => date("Y-m-d H:i:s", time())
            ], [
                'discount_id' => $discount_id
            ]);
            if ($retval == 1) {
 ...
 */
    ...
    public function setStatusPromotionMansong($mansong_id,$type){
        $promotion_mansong = new NsPromotionMansongModel();
		$arr=['examine_status' => $type,'modify_time' => date("Y-m-d H:i:s", time())];
		if($type==1){$arr['status']=1;}
        $retval = $promotion_mansong->save( $arr , [
            'mansong_id' => $mansong_id
        ]);
        return $retval;
    }
    ...
    public function setStatusPromotionDiscount($discount_id,$type){
		...
		if($type==1){ $arr=['examine_status' => $type,'status' =>1]; } else { $arr=['examine_status' => $type]; }
		$arr['modify_time']=date("Y-m-d H:i:s", time());
		if($type==1){
			$prodisgoods=new NsPromotionDiscountGoodsModel();
			$prodisgoods->save(['status' =>1], ['discount_id' => $discount_id]);
		}
        $retval = $promotion_mansong->save($arr, ['discount_id' => $discount_id]);
        return $retval;
    }
