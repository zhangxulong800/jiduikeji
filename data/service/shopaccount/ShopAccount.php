...
class ShopAccount extends BaseService
{
     ...
     */
    public function addShopAccountProceedsRecords($serial_no, $shop_id, $money, $account_type, $type_alis_id, $remark, $title)
    {
        ...
        if (empty($records_list)) {
            ...
            try {
                ...
                // 更新店铺总收益字段
                $this->updateShopAccountProceeds($shop_id, $money, $account_type);
                // 添加店铺的整体资金流水
                $this->addShopAccountRecords(getSerialNo(), $shop_id, $money, $account_type, $type_alis_id, $remark, $title);
                $model->commit();
                return $model->id;
            } catch (\Exception $e) {
                $model->rollback();
     ...
     * @param unknown $money            
     */
    private function updateShopAccountProceeds($shop_id, $current_num, $account_type)
    {
        $account_model = new NsShopAccountModel();
        $account_info = $account_model->get($shop_id);
        // 没有的话新建账户
        ...
        if($account_type == 2){
            $data = array(
                "shop_proceeds" => $account_info["shop_proceeds"] + $current_num
            );
        }
        if($account_type == 1){
            $data = array(
                "shop_point" => $account_info["shop_point"] + $current_num
            );
        }
        $retval = $account_model->save($data, [
            'shop_id' => $shop_id
     ...
     * @param unknown $remark            
     */
    public function withdrawShopAccountRecords($rec_type,$withdraw_no, $shop_id, $money,$data_id, $remark,$nowmoney,$nowpoint)
    {
		...
		try {
			$data = array(
				...
				'money' => -$money,
				'data_id' => $data_id,
				'shop_money' => $nowmoney-$money,
				'shop_point' => $nowpoint,
				'text' => $remark,
				'create_time' => date("Y-m-d H:i:s", time())
			);
			$records_id = $model->save($data);// 添加店铺的整体资金流水
			// 更新店铺金额
			$accountdata = array('money'=>$nowmoney-$money);
			$Shop->save($accountdata, [
                'shop_id' => $shop_id
            ]);
    ...
    /**
     * 添加店铺的整体的资金流水，视情形加入总额
     *
     */
    public function addShopAccountRecords($rec_type, $shop_id, $data_no, $data_id, $money, $point, $text, $is_add = 1)
    {
        $model = new NsShopAccountRecordsModel();
        $condition['shop_id'] = $shop_id;
        $shop = (new NsShopModel())->get($condition);
        ...
        $condition = array(
            ...
            'data_id' => $data_id,
            'shop_id' => $shop_id);
        $data = $model->get($condition);
		/*推广积分是加入extend_point另外一个总数里的*/
		if($is_add == 0 || $rec_type==9){$tot_point=$shop->point;$tot_money=$shop->money;} else {$tot_point=$shop->point + $point;$tot_money=$shop->money + $money;}
        if(empty($data)){
            $data = array(
                'rec_type' => $rec_type,
                'data_no' => $data_no,
                ...
                'point' => $point,
                'money' => $money,
                'shop_money' => $tot_money,
                'shop_point' => $tot_point,
                'create_time' => date("Y-m-d H:i:s", time()),
                'text' => $text,
                'is_add'=> $is_add);
            $model->save($data);
    ...
    public function getAccountRecordSum($shop_id = '', $account_type, $start_time='', $end_time='', $is_add = '')
    {
        ...
        $shop_account = new NsShopAccountRecordsModel();
        $account = $shop_account->getSum($condition, $field);
        if(empty($account)){
            $account = 0;
        }
        return $account;
