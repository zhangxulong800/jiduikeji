...
class GoodsDiscount extends BaseService{
    ...
    public function getDiscountGoodsList($page_index = 1, $page_size = 0, $condition = array(), $order = ''){
        ...
        if(!empty($goods_list['data']))
        {
            foreach ($goods_list['data'] as $k => $v)
            {
                ...
                $goods_list['data'][$k]['discount_id'] = $v['discount_id'];
                ...
                $goods_list['data'][$k]['stock'] = (new Goods())->getGoodsSkuStock($v['goods_id']);
            }
        }
        return $goods_list;
    }
    ...
    /**
     * 获取 一个商品的 限时折扣信息
     * 总后台统一配置添加时直接影响PromotionDiscountGoods是否可用
     */
    public function getDiscountByGoodsid($goods_id){
        $discount_goods = new NsPromotionDiscountGoodsModel();
		$nowdate=date("Y-m-d H:i:s", time());
		$condition['start_time'] = ['<',$nowdate];
		$condition['end_time'] = ['>',$nowdate];
		$condition['goods_id'] = $goods_id;
		$condition['status'] = 1;
		$discount = $discount_goods->where($condition)->find();
		if(empty($discount)){
			return -1;
		} else {return $discount['discount'];}
    }
	/*获取 一个商品的 限时折扣对应的信息discount_goods_id*/
    public function getDiscountGoodsidByGoodsid($goods_id){
        $discount_goods = new NsPromotionDiscountGoodsModel();
        $discount = $discount_goods->getInfo(['goods_id'=>$goods_id, 'status'=>1], 'start_time,end_time,discount,discount_goods_id');
		if(empty($discount)){
			return -1;
		}
		$nowtime= strtotime(date("Y-m-d H:i:s", time()));
		if(strtotime($discount['start_time'])<$nowtime && strtotime($discount['end_time'])>$nowtime){
			return $discount['discount_goods_id'];
		} else {return -1;}
    }
    ...
    public function getDiscountByName($discount_name){
        $discount_discount = new NsPromotionDiscountModel();
        $condition = "discount_name like '%".$discount_name."%' and status=1";

        $discount_id = $discount_discount->getInfo( $condition, 'discount_id');
     ...
     * @param unknown $goods_id
     */
    public function getDiscountInfoByID($discount_id){
        $discount_discount = new NsPromotionDiscountModel();
        $discount_info = $discount_discount->getInfo( ["discount_id"=>$discount_id]);
     ...
     */
	/*
    public function discountPriceByGoodsid($shop_id,$goods_id,$price){
        $siteconf=new Config();
		$onekey=$siteconf->getConfig(0,'OPERATE_CONFIG');//总后台统一配置
		$onearr=json_decode($onekey['value'],true);
		if($onearr['is_discount_open']){
			//获取商家的折扣配置信息
			$onedisc=Db::table('ns_promotion_discount_goods')->where(['shop_id'=>$shop_id,'goods_id'=>$goods_id,'status'=>1])->find();
			if(empty($onedisc)){ $discount_price = 0;}
			else {
				$nowtime= strtotime(date("Y-m-d H:i:s", time()));
				if(strtotime($onedisc['start_time'])<$nowtime && strtotime($onedisc['end_time'])>$nowtime){
					$discount_price = sprintf("%.2f",round($price*($onedisc['discount']/10),2));
				} else { $discount_price = 0;}
			}
		} else { $discount_price = 0; } //为0表示不存在折扣价格
