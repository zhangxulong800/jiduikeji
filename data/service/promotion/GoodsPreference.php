...
class GoodsPreference extends BaseService{
    ...
    public function getGoodsSkuPrice($sku_id)
    {
        ...
        $goods_sku_info = $goods_sku->getInfo(['sku_id' => $sku_id], 'goods_id,market_price,price,promote_price');
		...
		if($point_exchange_type==2){
			...
			//if($price > $goods_sku_info['promote_price']){$price = $goods_sku_info['promote_price'];}
			if(!empty($this->uid))
			{
				//$member_price = $this->getGoodsSkuMemberPrice($sku_id, $this->uid);原来的暂注释
				//销售价与JPlus价格比较，该项目会员价用
				$member = NsMemberModel::get($this->uid);
				if($member['jplus_level'] > 0){
					if($price > $goods_sku_info['market_price']){$price = $goods_sku_info['market_price'];}
				}
			}
			/*判断有无有效的折扣价格，此处只折扣无团购*/
			$goodsDiscount=new GoodsDiscount();
			$discount=$goodsDiscount->getDiscountByGoodsid($goods_sku_info['goods_id']);//是否存在
			if($discount > 0){
				$discount_price=$goods_sku_info['price']*($discount/10);
  ...
  /*   public function getPointMoney($point, $shop_id)
    ...
    public function getGoodsSkuListGoods($goods_sku_list){
        ...
        if(!empty($goods_sku_list)){
            ...
            foreach ($goods_sku_list_array as $k => $v)
            {
                ...
                $array[] = array($goods_id, $sku_item[0], $sku_item[1],$shop_id);
            }
    ...
    public function getMemberCouponList($goods_sku_list)
    {
        ...
        $member_coupon_list = $member_coupon->getUserCouponList(1,$goods_sku_list_array[0][3]);
        //2.获取当前优惠券是否可用
    ...
    public function getGoodsSkuMemberPrice($goods_sku_id, $uid){
        //查询sku相关信息
        $goods_sku = new NsGoodsSkuModel();
        $sku_info = $goods_sku->getInfo(['sku_id'=> $goods_sku_id], 'price, market_price');
        $member_level_discount = $this->getMemberLevelDiscount($uid);
        $member = NsMemberModel::get($uid);
        if($member['jplus_level'] > 0){
            return $sku_info['market_price'];
        }
