...
class Member extends User implements IMember
{
    ...
    public function registerMember($user_name, $password, $email, $mobile, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid, $pid, $path_pid )
    {
		...
		if ($res > 0) {
            ...
            $data = $coupon_type->getInfo($condition, '*');
            if(!empty($data)){
                $this->memberGetCoupon($res, $data["coupon_type_id"], 1, $data["valid_days"]);
                $this->memberGetCoupon($res, $data["coupon_type_id"], 1, $data["valid_days"]);
            }

            sendSmsByUid($res,
                '恭喜您成为积分呗会员，您获得了100积分红包，免费兑换平台商品，微信搜索公众号“jiduikeji”或“积兑科技”享更多优惠。');
     ...
     *
    */
	public function experiencePoint($uid){
		$reg_time=Db::table('ns_member')->where('uid',$uid)->value('reg_time');
		$cha=time()-strtotime($reg_time);
		if($cha>3*24*60*60){  //3天体验期
			$experiencePoint=0;
		} else {
			$used=0;
			$useredArr=Db::table('ns_member_account_records')->where(['uid'=>$uid,'account_type'=>2,'from_type'=>31])->select();
			foreach($useredArr as $key=>$val){
				$textArr=explode('-',$val['text']);
				if($textArr[0]=='新注册体验通证红包')
				{
				    $used+=$textArr[1];
				}
			}
			$experiencePoint=1000-$used;//体验通证积分初始1000
		}  
     ...
     * @param unknown $pid
     */
    public function getPidNum($uid)
    {
        $res = Db::table('ns_member')->where('pid', $uid)->count('pid');
        return $res;
    }
    /**
     * 通过用户id获取其通兑积分数量
     ...
     * @param unknown $pid
     */
    public function getExcPoint($uid)
    {
        $res = Db::table('ns_member_account')->where('uid',$uid)->column('exc_point');
        return $res;
    }
    /**
     * 通过用户id获取其通兑积分总积分
     ...
     * @param unknown $pid
     */
    public function getExcPointNum($uid)
    {
        $condition['uid'] = $uid;
        $condition['number'] = ['>',0];
        $res = Db::table('ns_give_point_records')
            ->where($condition)
            ->sum('number');
        print_r($uid);exit;
        return $res;
 ...
 */
    ...
    {
        $member = new NsMemberModel();
        $res= $member->getQuery(['pid'=>$uid],'uid','');
        return $res;
    }
    /**
     * 给用户id前面拼接0变为8位数
     ...
     * @param unknown num
     */
    public function setUidZero($uid)
    {
    ...
    public function getAccountList($page_index, $page_size, $condition, $order = '', $field = '*')
    {
        $member_account = new NsMemberAccountRecordsViewModel();
        if($order == ''){
            $order = 'nmar.create_time desc';
        }
        $list = $member_account->getViewList($page_index, $page_size, $condition, $order);
        if (! empty($list['data'])) {
            foreach ($list['data'] as $k => $v) {
                $list['data'][$k]['type_name'] = MemberAccount::getMemberAccountRecordsName($v['from_type']);
                $list['data'][$k]['title'] = MemberAccount::getMemberAccountRecordsIsAddName($v['is_add']);
            }
        }
        return $list;
    }
    ...
    public function getMemberDetail($shop_id = '')
    {
        ...
        if (! empty($this->uid)) {
            ...
            //jplus_status：0、非会员；10、临时；20、正式
            $member_info['jplus_status'] = 0;
            if(strtotime($member_info['assign_jplus_time']) > strtotime('2018-12-01')){
                if (strtotime($member_info['assign_jplus_time']) >= strtotime("-1 years",date("Y-m-d", time()))) {
                    $member_info['jplus_status'] = 20;
                }
                $member_info['assign_jplus_time'] = date('Y-m-d', strtotime("+1 years", strtotime($member_info['assign_jplus_time'])));
            }
            else{
                $member_info['assign_jplus_time'] = '2018-12-01';
                if(strtotime($member_info['reg_time']) >= strtotime("-3 days",date("Y-m-d", time()))){
                    $member_info['jplus_status'] = 10;//改为3天
                    $member_info['assign_jplus_time'] = date('Y-m-d', strtotime("+3 days", strtotime($member_info['reg_time'])));
                }
            }
            // 获取user信息
            ...
            $member_info["introduce_member"] = "";
            if(($member_info['pid']!= "")&&($member_info['pid']!= "0")){
                $introduce_member = (new UserModel())->getQuery('uid = '.$member_info['pid'], 'user_name', '');
                if(!empty($introduce_member)){
                    $member_info["introduce_member"] = $introduce_member[0]['user_name'];
                }
            }
            ...
            $member_info['point_gift'] = $member_account->getMemberPointGift($this->uid);
            $member_info['exc_point'] = $member_account->getMemberTdPoint($this->uid);
            $member_info['point_not_add'] = $member_account->getMemberAccountRecordSum('', $this->uid, 1,
                date('Y-m-d', strtotime('-30 days', time())), date('Y-m-d H:i:s', time()), '0');
    ...
    public function memberPointToBalance($uid, $shop_id, $point)
    {
        ...
        try {
            ...
            if ($point > $member_account_info['point']) {
                ...
                $point_info = $point_config->getInfo(['shop_id' => 0], 'is_open, convert_rate');//0替换$shop_id
    ...
    public function accountGiving($user_id, $account, $account_type = 1){
        $member_account_model = new NsMemberAccountModel();
        $member_account_model->startTrans();
        try {
            $member_account_info = $this->getMemberAccount($this->uid, 0);
            $account_remain = $member_account_info['point'];
            if($account_type ==  2){
                $account_remain = $member_account_info['balance'];
            }
            if ($account > $account_remain) {
                $member_account_model->commit();
                return LOW_BALANCE;
            } else {
                $member_account = new MemberAccount();
                $retval = $member_account->addMemberAccountData(0, 1, $user_id, 1, $account , 30, 0, '受让积分');
                if ($retval < 0) {
                    $member_account_model->rollback();
                    return $retval;
                }
                $retval = $member_account->addMemberAccountData(0, 1, $this->uid, 0, $account * (- 1), 30, 0, '转让积分');
                if ($retval < 0) {
                    $member_account_model->rollback();
                    return $retval;
                }
    ...
    public function memberBalanceToPoint($uid, $shop_id, $balance)
    {
        $member_account_model = new NsMemberAccountModel();
        $member_account_model->startTrans();
        try {
            $member_account_info = $this->getMemberAccount($uid, $shop_id);
            if ($balance > $member_account_info['balance']) {
                $member_account_model->commit();
                return LOW_BALANCE;
            } else {
                    $member_account = new MemberAccount();
                    $exchange_point = $member_account->balanceToPoint($balance, $shop_id);
                    $retval = $member_account->addMemberAccountData($shop_id, 1, $uid, 0, $exchange_point , 3, 0, '余额通兑积分');
                    if ($retval < 0) {
                        $member_account_model->rollback();
                        return $retval;
                    }
                    $retval = $member_account->addMemberAccountData($shop_id, 2, $uid, 1, $balance * (- 1), 3, 0, '余额通兑积分');
                    if ($retval < 0) {
                        $member_account_model->rollback();
                        return $retval;
                    }
    ...
    public function assignMember($jplus_level)
    {
        $account_info = $this->getMemberAccount($this->uid, 0);
        $account = 0;
        if($jplus_level == 10){
            $account = 399;
        }
        ...
        if($account_info['balance'] < $account){
            return -4;
        }

        $member_account = new MemberAccount();
        $retval = $member_account->addMemberAccountData(0, 2, $this->uid, 0, -1 * $account, 25, 0, '购买JPlus会员');
        if($retval != 1){
            return $retval;
        }
        $member = NsMemberModel::get($this->uid);

        $path_pid = $member['path_pid'];
        $path_arr = explode('#', $path_pid);
        $path_count = count($path_arr);

        $member['assign_jplus_time'] = date("Y-m-d H:i:s", time());

        $member['jplus_level'] = $jplus_level;
        if($jplus_level == 20) {
            $member['grade'] = 5;
        }
        $member->save();
        ...
        if($jplus_level == 10) {
            $retval = $member_account->addMemberAccountData('', 1, $this->uid, 1, 30, 25, 0, '本人购买JPlus会员获赠积分');
        }

        if($jplus_level == 20) {
            $retval = $member_account->addMemberAccountData('', 1, $this->uid, 1, 650, 25, 0, '本人购买JPlus会员获赠积分');
        }
		/*给云知检信息科技20积分*/
		$shop_account = new ShopAccount();
        $shop_account->addShopAccountRecords(7, 44, 0,$this->uid, 0, 20, '会员兑换增值服务', 1); //rec_type类型调为7
        ...
        for ($i = $path_count - 1; $i >= 0; $i--) {
            ...
            $grade = $this->getMemberInfo($path_arr[$i]);
            ...
            $retval = $member_account->addMemberAccountData(0, 2, $path_arr[$i], 1, $gift, 25, $this->uid, '他人购买JPlus会员收益');//添加购买人uid信息，为获取对应信息文本不可变
            if($retval != 1){
                return $retval;
            }
        }
		/*预提数据加入用户账户开始*/
        $member_account_record = new NsMemberAccountRecordsModel();//满一个月未购买jplus会删除预生成记录
		$condition = array('uid'=> $this->uid,'is_add'=>0);
		$condition['from_type'] = array('in','15,35,20,40,55'); //25不包括，55是推广积分；未购买jplus预先生成的为0的记录
		$list = $member_account_record->getQuery($condition, '*', 'create_time desc');
        $account_statistics = new NsMemberAccountModel();
        $order = new NsOrderModel();
		$userdata = $account_statistics->get(['uid'=>$this->uid]);
        foreach ($list as $item){
            if($item['from_type'] ==15 || $item['from_type'] == 35 || $item['from_type'] == 55){  //下级会员买商品的分销权益及领取通证红包必须自身是jplus会员时才计入
                $order_detail = $order->get(['order_id' => $item['data_id']]);
                if($order_detail['order_status'] == 4){
					$member_account_record->where('id',$item['id'])->update(['text'=>$item['text'].'。预生成时间:'.$item['create_time'],'create_time'=>date('Y-m-d H:i:s', time()),'is_add'=>1]);
					if($item['account_type']==1){
						if($item['from_type']==55){  //55推广积分，account_type一定是1
							$account_statistics->where('id',$userdata['id'])->update(['extend_point'=>$userdata['extend_point']+$item['number']]);
						} else {
							$account_statistics->where('id',$userdata['id'])->update(['point'=>$userdata['point']+$item['number']]);
						}
					}
					else {
						$account_statistics->where('id',$userdata['id'])->update(['balance'=>$userdata['balance']+$item['number']]);
					}
                }
				//在退货及订单关闭环节删除预先生成的记录
            }
        ...
        $data = $coupon_type->getInfo($condition, '*');
        if(!empty($data)){
            $this->memberGetCoupon($this->uid, $data["coupon_type_id"], 1, $data["valid_days"]);
        }
        ...
        sendSmsByUid($this->uid,
            '恭喜您成为积分呗Jplus'.$sms_info.'会员,赶紧登陆领取您的海量福利,包括价值'.$sms_info.'赠品一份(领取支付时请选择自动赠送'.$sms_info.'满减优惠券)！有任何疑问请及时拨打0731- 8990 8931联系客服哦~！');
    ...
    public function createMemberRecharge($recharge_money, $uid, $out_trade_no)
    {
        ...
        if ($res) {
            $pay = new UnifyPay();
            $pay->createPayment(0, $out_trade_no, '余额充值', '用户余额充值', $recharge_money, 4, $member_recharge->id);
        }
        return $res;
    }
    ...
    public function payMemberRecharge($out_trade_no){
		...
		//执行该函数时是已经支付，无需判断order_payment是否支付成功
		$racharge_obj = $member_recharge_model->where('out_trade_no',$out_trade_no)->find();//充值只生成一个单
		if (! empty($racharge_obj)) {
			...
			$member_account = new MemberAccount();//下面第一个店铺id为0，只是填充满足共用函数需要。充值不需要知道shop_id
			$member_account->addMemberAccountData(0, 2, $racharge_obj["uid"], 1, $racharge_obj["recharge_money"], 4, $racharge_obj["id"], "余额充值!");
		}
    }
    ...
    public function addMemberBalanceWithdraw($shop_id, $withdraw_no, $uid, $bank_account_id, $cash)
    {
        ...
        if($balance_withdraw->getsum($condition, 'cash') + $cash >= 5000){
            return USER_WITHDRAW_MAX_SUM;
        };

        if($balance_withdraw->getCount($condition, 'cash') >= 3){
            return USER_WITHDRAW_MAX_TIMES;
        };
    ...
    public function MemberBalanceWithdrawAudit($shop_id, $id, $status)
    {
        ...
        $retval = $member_balance_withdraw->where(array(
            ...
            "status" => $status,"payment_date" => date('Y-m-d H:i:s', time())
        ));
        $member_balance_withdraw = new NsMemberBalanceWithdrawModel();
     ...
     */
    public function memberGetCoupon($uid, $coupon_type_id, $get_type, $valid_days = 0)
    {
        if ($get_type == 2) {
        ...
        $member_coupon = new MemberCoupon();
        $retval = $member_coupon->UserAchieveCoupon($uid, $coupon_type_id, $get_type, $valid_days);
        return $retval;
    }
