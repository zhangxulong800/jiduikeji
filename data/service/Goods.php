...
class Goods extends BaseService implements IGoods
{
    ...
    public function getGoodsList($page_index = 1, $page_size = 0, $condition = '', $order = '')
    {
		...
		$serch_status=0;
		if(isset($_POST['sear_name']) || isset($_POST['sear_type']) || isset($_POST['order_state']) || isset($_POST['controlType']) || isset($_POST['shop_id'])){
			$serch_status=1; //判断是否是搜索情形
		}
        // 针对商品分类
        if (! empty($condition['ng.category_id'])) {
            ...
            if(!empty($goods_list) && count($goods_list)>0){
				...
				if($serch_status){
					$condition=" ng.goods_id in (".$query_goods_ids.") and ng.group_id_array not in (114,115,117) and ( ng.category_id in (".$category_list.") or ".$extend_query.")";//搜索过滤
				} else {
		...
		if($serch_status){
			if(is_array($condition)){
				$condition["group_id_array"] = ["not in",[114,115,117]];//两类搜索过滤
			} 
		}
     ...
     * 0元拼团配置
     */
    function modifyGoodsAssemble($goodsId, $zero_point_num, $zero_num, $zero_state)
    {
     ...
     */
    public function addOrEditGoods($goods_id, $goods_name, $shopid, $category_id, $category_id_1, $category_id_2, $category_id_3,
                                   $supplier_id, $brand_id, $group_id_array, $goods_type, $market_price, $price, $cost_price,$cost_price_logistics,
                                   $point_exchange_type, $point_exchange, $give_point, $is_member_discount, $shipping_fee, $shipping_fee_id, $stock,
                                   $max_buy, $min_stock_alarm, $clicks, $sales, $collects, $star, $evaluates, $shares, $province_id, $city_id,
                                   $picture, $keywords, $introduction, $description, $QRcode, $code, $is_stock_visible, $is_hot, $is_recommend,
                                   $is_new, $sort, $image_array, $sku_array, $state, $sku_img_array, $goods_attribute_id, $goods_attribute,
                                   $goods_spec_format, $goods_weight, $goods_volume, $shipping_fee_type, $extend_category_id, $sku_picture_values)
    {
        $error = 0;
        ...
        try {
            $data_goods = array(
                ...
                'cost_price_logistics' => $cost_price_logistics,
            ...
            if($goods_type == 1){
                $data_goods['total_cost_price'] = $cost_price_logistics+($market_price-$cost_price)*0.05+$price*0.05;//Jplus-结算的5%为税 销售价的5%为损耗,1.2%为财务成本
            }else{
                $data_goods['total_cost_price'] = $cost_price_logistics+($market_price-$cost_price)*0.05+$price*0.012+$price*0.01;//Jplus-结算的5%为税,1.2%为财务成本,1%为销售成本
            }
            if ($goods_id == 0) {
                ...
                if (! empty($sku_array)) {
                    ...
                    foreach ($sku_list_array as $k => $v) {
                        $res = $this->addOrUpdateGoodsSkuItem($this->goods->goods_id, $v, $cost_price_logistics, $goods_type);
                        if (! $res) {
                            $error = 1;
                        }
                    }
                    ...
                    if($goods_type == 1){
                        $sku_data['total_cost_price'] = $cost_price_logistics+($market_price-$cost_price)*0.05+$price*0.05;//Jplus-结算的5%为税 销售价的5%为损耗,1.2%为财务成本
                    }else{
                        $sku_data['total_cost_price'] = $cost_price_logistics+($market_price-$cost_price)*0.05+$price*0.012+$price*0.01;//Jplus-结算的5%为税,1.2%为财务成本,1%为销售成本
                    }
                ...
                if (! empty($sku_array)) {
                    ...
                    foreach ($sku_list_array as $k => $v) {
                        $res = $this->addOrUpdateGoodsSkuItem($goods_id, $v, $cost_price_logistics, $goods_type);
                        if (! $res) {
                            $error = 1;
                        }
                    }
     ...
     */
    public function getGoodsSkuStock($goods_id)
    {
        $goods_sku = new NsGoodsSkuModel();
        ...
        foreach ($goods_sku_detail as $k => $goods_sku) {
            $goods_sku_stock = $goods_sku_stock + $goods_sku['stock'];
        }

        return $goods_sku_stock;
    }

        public function getGoodsDetail($goods_id, $admin = 0)
    {  
        // 查询商品主表
        $goods = new NsGoodsModel();
        $goods_detail = $goods->get($goods_id);
		...
		$original_price=$goods_detail['price'];//最初的价格
		/*
        $goods_preference = new GoodsPreference();
        if (! empty($this->uid)) {
	...
	/*
        ...
        //将销售价修改为JPlus价格
        $goods_detail["jplus_level"] = 0;
        if (! empty($this->uid)) {
            $member = NsMemberModel::get($this->uid);
            if($member['jplus_level'] > 0){
                $goods_detail["jplus_level"] = $member['jplus_level'];//传到后面用
				/* 最上面不显示jplus价格
				if($goods_detail['price'] > $goods_detail['market_price']){
					$goods_detail['price'] = $goods_detail['market_price'];
				} */
            }
        }

        if($admin != 1){
            /*判断有无有效的折扣价格，此处只折扣无团购，折扣以原始price为准*/
            $goodsDiscount=new GoodsDiscount();
            $discount=$goodsDiscount->getDiscountByGoodsid($goods_id);//是否存在
            if($discount > 0){
                $discount_price=$original_price*($discount/10);
                if($discount_price < $original_price){
                    $goods_detail['price']=sprintf("%.2f",round($discount_price,2));
					$goods_detail['promotion_info'] ='团购价';
                }
            }
        }
		/*原来的goods表中保护的活动信息后台存入不准确，弃用
        $goods_preference = new GoodsPreference();
        ...
        if (! empty($goods_promotion_info)) {
            ...
            $goods_detail['promotion_detail'] = $goods_discount_info->getInfo(['discount_id' => $goods_detail['promote_id']
            ], 'start_time, end_time');//该表含限时折扣和团购信息
        }
        ...
        foreach ($goods_sku_detail as $k => $goods_sku) {
			...
			$first_price=$goods_sku_detail[$k]['price'];
			if($goods_detail['jplus_level'] > 0){
				if($goods_sku_detail[$k]['price'] > $goods_sku['market_price']){
					$goods_sku_detail[$k]['price'] = $goods_sku['market_price'];
				}
                //$goods_sku_detail[$k]['member_price'] = $goods_sku['market_price'];
            }
			if($discount > 0){
				$discount_price=$first_price*($discount/10);
				if($discount_price < $goods_sku_detail[$k]['price']){
					$goods_sku_detail[$k]['price']=sprintf("%.2f",round($discount_price,2));
				}
			}
        }
     ...
     */
    private function addOrUpdateGoodsSkuItem($goods_id, $sku_item_array,$cost_price_logistics, $goods_type)
    {
        ...
        if (empty($sku_count)) {
            ...
            if($goods_type == 1){
                $data['total_cost_price'] = $cost_price_logistics+($sku_item[2]-$sku_item[3])*0.05+$sku_item[1]*0.05+$sku_item[1]*0.012;//Jplus-结算的5%为税 销售价的5%为损耗,1.2%为财务成本
            }else{
                $data['total_cost_price'] = $cost_price_logistics+($sku_item[2]-$sku_item[3])*0.05+$sku_item[1]*0.012+$sku_item[1]*0.01;//Jplus-结算的5%为税,1.2%为财务成本,1%为销售成本
            }
            $goods_sku->save($data);
    ...
    public function getCartList($carts)
    {
        ...
        foreach ($cart_list as $k => $v) {
            ...
            $goods_info = $goods->getInfo([
                'goods_id' => $v['goods_id']
            ], 'max_buy,state,point_exchange_type,point_exchange, group_id_array');
            // 获取商品sku信息
            $goods_sku = new NsGoodsSkuModel();
            $sku_info = $goods_sku->getInfo([
                'sku_id' => $v['sku_id']
            ...
            $v['group_id_array'] = $goods_info['group_id_array'];
...
//					$price = $onemem['is_jplus']==1 ? $sku_info["market_price"] : $price;//market_price代替jplus_price
                    //将销售价修改为JPlus价格
                    if($onemem['jplus_level'] > 0){
                        $price = $sku_info["market_price"];
                    } */
					$price_arr=$this->getLowestPrice($uid,$v['sku_id'],$sku_info['price'],$v['num']);
                    ...
                    $update_data = array(
						 ...
						 "num"=>$price_arr['num'],
						 "bl_id"=>$price_arr['bl_id']
                     );
                     //更新数据 
                     $cart->save($update_data, ["cart_id"=>$v["cart_id"]]);
					 Db::table('ns_cart')->where(['cart_id'=>$v["cart_id"]])->update(["num"=>$price_arr['num']]);
                     $cart_goods_list[$k]["price"] = $price_arr['price'];
					 $cart_goods_list[$k]["num"] = $price_arr['num'];
    ...
    public function addCart($uid, $shop_id, $shop_name, $goods_id, $goods_name, $sku_id, $sku_name, $price, $num, $picture, $bl_id)
    {
        ...
        if($uid > 0){
			...
			$price_arr=$this->getLowestPrice($uid,$sku_id,$price,$num);//获取适用的最低价格
			$price=$price_arr['price'];
			$bl_id=$price_arr['bl_id'];
			$num=$price_arr['num'];
	...
	/*获取适用的最低价格*/
	public function getLowestPrice($uid,$sku_id,$price,$num)
    {	
		$goods_sku=Db::table('ns_goods_sku')->where(['sku_id'=>$sku_id])->find();
		$zero_state = Db::table('ns_goods')->where(['goods_id'=>$goods_sku['goods_id']])->field('zero_state')->find();

		if($price > $goods_sku['price']){$price = $goods_sku['price'];}
		//将销售价修改为JPlus价格
		$member = NsMemberModel::get($uid);
		if($member['jplus_level'] > 0){
			if($price > $goods_sku['market_price']){$price = $goods_sku['market_price'];}
		}
		if($zero_state['zero_state'] == 1){
            $price = $goods_sku['price'];
        }
		/*判断有无有效的折扣价格；一个团购期只能买一件*/
		$goodsDiscount=new GoodsDiscount();
		$discount=$goodsDiscount->getDiscountByGoodsid($goods_sku['goods_id']);//是否存在
		$bl_id=0;
		if($discount > 0){	
			$discount_price=$goods_sku['price']*($discount/10);
			if($price >= $discount_price){
				$price = $discount_price;
				$bl_id='discount_goods_id-'.$goodsDiscount->getDiscountGoodsidByGoodsid($goods_sku['goods_id']);
				$num=1; //限购一件
			}
		} 
		return array('price'=>$price,'num'=>$num,'bl_id'=>$bl_id);
	}
	/*获取团购限购已购商品数量，返回相应的信息*/
	public function alreadyByInfo($uid,$goods_id)
    {	
		$goodsDiscount=new GoodsDiscount();
		$bl_id='discount_goods_id-'.$goodsDiscount->getDiscountGoodsidByGoodsid($goods_id);
		$cart_num=Db::table('ns_cart')->where(['buyer_id'=>$uid,'goods_id'=>$goods_id,'bl_id'=>$bl_id])->sum('num');
		
		$order_num=0;
		$order_goods_arr=Db::table('ns_order_goods')->where(['buyer_id'=>$uid,'goods_id'=>$goods_id,'memo'=>$bl_id])->field('order_id,num,refund_real_money')->select();
		$nowtime=time();
		$day15time=15*24*60*60;//设置10天内的订单计入。NsPromotionDiscountModel清空memo可能重复
		$adaytime=1*24*60*60;//代码要考虑第一次下单后7天内准确可以下第二单
		$nearest_buy_time=0;
		foreach($order_goods_arr as $k=>$v){
			$order_one=Db::table('ns_order')->where(['order_id'=>$v['order_id']])->field('order_status,create_time')->find();
			$chatime=$nowtime-strtotime($order_one['create_time']);
			if($order_one['order_status'] <> 5 && $v['refund_real_money']==0 && $chatime<$day15time){
				$order_num+=$v['num'];
				if($nearest_buy_time < strtotime($order_one['create_time'])){$nearest_buy_time = strtotime($order_one['create_time']);}
			}
		}
		$jplus_level=Db::table('ns_member')->where(['uid'=>$uid])->value('jplus_level');
		$tot_num=$cart_num+$order_num;
		if($jplus_level > 0){
			if($tot_num >=1 ){
				$arr=array('num'=>0,'message'=>'你已购买超过1件不能再购！');
			} 
			/*
			elseif($tot_num ==1){
				if($cart_num==1){$arr=array('num'=>0,'message'=>'购物车中存在请进入结算购买！');}
				else {
					if($nearest_buy_time > 0 && ($nowtime-$nearest_buy_time)<$adaytime){
						$arr=array('num'=>0,'message'=>'一天内只能团购一件！');
					} else {$arr=array('num'=>1,'message'=>'可以复购一件！');}
				}
			} */
			else {$arr=array('num'=>1,'message'=>'可以团购一件！');}
			
		} else {
			if($tot_num >= 1){$arr=array('num'=>0,'message'=>'已参加团购不能购买！');}
			else {$arr=array('num'=>1,'message'=>'可以团购一件！');}
		}
		return $arr;
	}
     ...
     * @Author zxl
     */
    public function getZeroGoodsList($page_index = 1, $page_size = 0, $where = '', $order = '')
    {
        $where = array(
            'shop_id'           => 0,
            'group_id_array'    => 117,
        ...
        $where['zero_num']          = array('neq', 0);
        $where['zero_point_num']    = array('neq', 0);
        $where['zero_state']        = array('eq', 1);

        $goodsList  = new NsGoodsModel();
        $result     = $goodsList->pageQuery($page_index, $page_size, $where, $order, '*');
        foreach ($result['data'] as $k => $v) {
    ...
    /**
     *获取满足首单免邮的毛利率大于40%的商品
     * @Author zxl
     */
    public function getMianGoodsList($page_index = 1, $page_size = 0, $where = '', $order = '', $goods_arr = '')
    {
        if($goods_arr == 1){//首页专用的三款商品 885是Hoo化妆品 654是体脂称  510是被子
            $where = [
                'goods_id' => ['in','510,823,885'],
                'group_id_array' => 0,
                'goods_type' => 1,
                'stock' => ['>', 0],
                'state' => 1
            ];
            $where['first_free_num'] = array('<>', 0);
            $where['market_price'] = array('>',"cost_price+cost_price*0.4");
        }else{
            $where = [
                'group_id_array' => 0,
                'goods_type' => 1,
        ...
        foreach ($result['data'] as $k => $v) {
            ...
            $result['data'][$k]['picture_info'] = $pic_info;
            $result['data'][$k]['type'] = 1;//此种情况是为了在商品详情页显示不同按钮
        }
        return $result;
    }
    ...
    public function getGoodsPointExchange($goods_id)
    {
        ...
        $goods_info = $goods_model->getInfo([
            'goods_id' => $goods_id
        ], 'shop_id,price,point_exchange_type,point_exchange');
        if ($goods_info['point_exchange_type'] == 0) {
            return 0;
        } elseif($goods_info['point_exchange_type'] == 2){
			$config = new NsPointConfigModel();
			$convert_rate=$config->where("shop_id",$goods_info['shop_id'])->value("convert_rate");
			return round($goods_info['price']/$convert_rate,2);
		} else {
     ...
     * 更改商品首单全免付款倍数
     * @param unknown $goods_id
     * @param unknown $first_free_num
     * @return boolean
     */
    public function updateGoodsFree($goods_id, $first_free_num){
        $goods = new NsGoodsModel();
        return $goods->save(['first_free_num'=>$first_free_num],['goods_id'=>$goods_id]);
    }
    /**
     * 更改商品排序
     * @param unknown $goods_id
    ...
    public function getMaxInterest($condition)
    {
        // TODO Auto-generated method stub
        $goods = new NsGoodsModel();
        $retval = $goods->getMax($condition, 'total_interest');
        return $retval;
    }
