...
use data\model\NsShopGroupModel;
...
class Shop extends BaseService implements IShop
{
	...
	/*店铺营业额信息*/
	public function shopBusiness($instance_id)
    {
		$condition['shop_id'] = $instance_id;
		$condition['order_status'] = 4;//算营业额只包括4
		$orders=Db::table('ns_order')->where($condition)->field('order_id,order_money,refund_money')->select();
		$total=0;
		foreach($orders as $key=>$val){
			$total=$total+$val['order_money']-$val['refund_money'];
		}
		return $total;
	}
	/*店铺待入账余额*/
	public function shopAvailable($instance_id)
    {
		$condition['shop_id'] = $instance_id;
		$condition['shipping_status'] = 0;
		$condition['order_status'] = array('in','1,-1');//发货就入账了商家
		$orders=Db::table('ns_order')->where($condition)->field('order_id')->select();
		$ava_cost_price=0;
		foreach($orders as $key=>$val){
			$order_goods=Db::table('ns_order_goods')->where('order_id',$val['order_id'])->field('cost_price,num,refund_real_money')->select();
			foreach($order_goods as $k=>$v){
				if($v['refund_real_money']==0){
					$ava_cost_price=$ava_cost_price+$v['cost_price']*$v['num'];
				}
			}
		}
		return $ava_cost_price;
	}
	/*店铺提现金额*/
	public function shopWithdraw($instance_id)
    {
		$condition['shop_id'] = $instance_id;
		$condition['status'] = 1; //1已同意
		$cash_tot=Db::table('ns_shop_withdraw')->where($condition)->sum('cash');
		return $cash_tot;
	}
    /**
     ...
     * zxl
     */
    public function givePoint($user_id, $point, $shop_id)
    {
        Db::startTrans();
        try {
            $shop_account_info = Db::table('ns_shop')->where('shop_id', $shop_id)->value('point');//获取商家的账户信息
            $member_account = new MemberAccount();
            $shop_point = Db::table('ns_shop')->where('shop_id', $shop_id)->update(['point' => ($shop_account_info - $point)]);

            $retvaluser = $member_account->addMemberAccountData(0, 1, $user_id, 1, $point, 30, 0, '商家转赠积分');

            $shop_account_records = Db::table('ns_shop_account_records')->where('shop_id', $shop_id)->order('create_time desc')->limit(1)->find();


            $data = array('rec_type' => 1, 'data_no' => 0, 'point' => $point, 'shop_id' => $shop_id, 'money' => 0, 'data_id' => $user_id, 'shop_money' => $shop_account_records['shop_money'], 'shop_point' => $shop_account_records['shop_point'] - $point, 'create_time' => date("Y-m-d H:i:s", time()), 'text' => '商家积分转赠给用户', 'is_add'=>1);
            $retvalshop = Db::table('ns_shop_account_records')->where('shop_id', $shop_id)->insert($data);
    ...
    */
    ...
    public function addshop($shop_name, $shop_type, $uid, $shop_group_id, $shop_company_name, $province_id, $city_id, $shop_address, $shop_zip, $shop_sort, $recommend_uid = 0)
    {
        ...
        try {
            ...
            $data = array(
                ...
                'shop_zip' => $shop_zip,
                'shop_sort' => $shop_sort,
				'shop_create_time' => date("Y-m-d H:i:s", time()),
                'recommend_uid' => $recommend_uid
            );
            // 添加店铺
     ...
     */
    public function updateShopConfigByshop($shop_id, $shop_logo, $shop_banner, $shop_avatar, $shop_qrcode, $shop_qq, $shop_ww, $shop_phone, $shop_keywords, $shop_description ,$shop_hint)
    {
        $data = array(
            'shop_logo' => $shop_logo,
     ...
     */
    public function updateShopConfigByPlatform($shop_id, $shop_name, $shop_group_id, $shop_platform_commission_rate,$service_charge_rate, $bond_collection_method, $bond, $shop_type,$shop_credit,$shop_desccredit,$shop_servicecredit,$shop_deliverycredit, $store_qtian, $shop_zhping, $shop_erxiaoshi, $shop_tuihuo, $shop_shiyong, $shop_shiti, $shop_xiaoxie, $shop_huodaofk, $shop_state, $shop_close_info)
    {
        $data = array(
            'shop_name' => $shop_name,
            'shop_group_id' => $shop_group_id,
            ...
            'bond_collection_method' => $bond_collection_method,
            'bond' => $bond,
            'shop_type' => $shop_type,
    ...
    {
        // 查询店铺的账户情况
        $shop_account_obj = $this->getShopInfo($shop_id);
        // 判断店铺金额是否够
        if ($shop_account_obj["money"] >= $cash) {
            ...
            if ($shop_account_withdraw->id > 0) {
                $shop_account_service = new ShopAccount();//rec_type为2，商家提现
                $retval = $shop_account_service->withdrawShopAccountRecords(2,$withdraw_no, $shop_id, $cash,$shop_account_withdraw->id, "店铺提现，扣除" . $shop_bank_account_info["realname"] . "店铺余额" . $cash,$shop_account_obj['money'],$shop_account_obj['point']);
            }
    ...
    {
		$count = Db::table('ns_order')->where($condition)->count();
        if ($page_size == 0) {
