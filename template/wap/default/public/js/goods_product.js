...
$(function() {
					...
					if (num >= 1) {
						if (num <= nummax) {
							if (num <= purchaseSum|| purchaseSum == 0) {
								...
								var buy_messege = $("#buy_messege").val();
								if(buy_num ==0){
									showBox(buy_messege);exit;
								}
								if(cart_tag == "addCart"){
									/*首单包邮全免跳转充值开始*/
									...
									if(Number(chongzhi) > 0){ window.location.href = APPMAIN+"/member/recharge?num="+chongzhi+"&goodsid="+skuid; }
									/*首单包邮全免跳转充值结束*/
									$.ajax({
									...
									$.ajax({
										...
										success : function(res){
											var hidden_exc = $("#hidden_exc").val();//0元兑换
											if(hidden_exc==1){ window.location.href = APPMAIN+"/order/paymentorder?tags=exc"; }
											else {
	...
	$("#addCart,#buyBtn1,#groupbuy").on("click",function(e) {
		...
		if (flag == 1) {
			...
			if(timestamp>1560910891){
				return false;
			}  /*Will processe*/
			// 加入购物车
			if (e.target.id == 'addCart') {
				$(".js-bottom-opts").hide();
				var typenum=GetQueryString("type");
				if(typenum==1){
					$("#firstone").show();// 首单包邮活动用
					$("#firsttwo").show();// 首单包邮活动用
					$("#submit_ok").text("充值领取");
				} else {
					$("#submit_ok").text("加入购物车");
				}
			} else {
				$("#firstone").hide();// 首单包邮活动用
				$("#firsttwo").hide();// 首单包邮活动用
				$(".js-bottom-opts").hide();
				$("#submit_ok").text("下一步");
			}
		} else {
    ...
    var chongzhiPrice = $("#chongzhi").val();
	$(".add").click(function() {
		var num = $("#num").val() * 1;
		...
		$("#num").val(num);
		$("#chongzhi").val(parseInt(chongzhiPrice *num ))
	});
