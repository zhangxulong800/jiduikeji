...
function loadFunction(){
	 ...
	 * @param {Object} even
	 */
	$('.evaluate_right_imgs').on('click','li',function(){
		var rate_content=$(this).parents('.rate_content');
		$(this).remove();  //必须写到前面不然等图片没了就无法移除！待后期验证先这样无问题
		var imgsrc=$(this).attr("data-img");
		$.ajax({
			type:"post",
			url:APPMAIN+"/components/deleteimgupload",
			data:{"imgsrc":imgsrc},
...
function UploadImage(event) {
	...
	$.ajaxFileUpload({
		url : APPMAIN+'/components/imgupload', // 用于文件上传的服务器端请求地址
		secureuri : false, // 一般设置为false
		fileElementId : file_upload, // 文件上传空间的id属性 <input type="file" id="file" name="file" />
		dataType : 'text', // 返回值类型 一般设置为json
		async : false,
		success : function(res){ // 服务器成功响应处理函数
			$('#'+file_upload).parents('.rate_content').find('.evaluate_right_imgs').append('<li style="background-image: url('+shop_main+'/' + res + ');" data-img="' + res + '"></li>');
...
function doSubmit(type){
	var ajaxUrl=APPMAIN+"/order/addgoodsevaluateagain";
	var flag = false;
	...
	$(".evaluate").each(function(i){
		...
		if(type==1){
			...
			ajaxUrl=APPMAIN+"/order/addgoodsevaluate";
		}
