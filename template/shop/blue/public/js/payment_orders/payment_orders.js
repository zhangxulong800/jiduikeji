...
$(function(){
	...
	$(".express_company").change(function(){
		var getval=$(this).children("option:selected").attr("data-express-fee");
		$(this).parent().prev().text("订单运费："+parseFloat(getval).toFixed(2));
		var tot_expr=0;
		$(".express_company").each(function(){
			var oneval=$(this).children("option:selected").attr("data-express-fee");
			tot_expr+=parseFloat(oneval);
		});
		$("#express").text(parseFloat(tot_expr).toFixed(2));
		$("#hidden_express").val(tot_expr);
	...
	$(".shipping_method").click(function(){
		...
		if(code=='ziti'){
			...
			$(this).next().show();
			$(this).text('快递物流');
			$(this).attr("data-code","kdwl");
			$(this).parent().find('.carriage').text("订单运费：0.00");
		} else {
			$(this).prev().show();
			$(this).next().hide();
			$(this).text('门店自提');
			$(this).attr("data-code","ziti");
			var once=$(this).parent().find('.express_company').children("option:selected").attr("data-express-fee");
			$(this).parent().find('.carriage').text("订单运费："+parseFloat(once).toFixed(2));
		}
		calculateTotalAmount();
	})
