...
class Order extends BaseController
{
    ...
    public function orderList()
    {
        if (request()->isAjax()) {
            ...
            $orderList = $order_service->getOrderList($page_index, $page_size, $condition, 'create_time desc');
			foreach($orderList['data'] as $key=>$val){
				$shop_shiti=Db::table("ns_shop")->where("shop_id",$val['shop_id'])->value('shop_shiti');
				$orderList['data'][$key]['shop_shiti']= $shop_shiti;
    ...
    public function orderDeliveryData()
    {
		...
		foreach($order_goods_list as $key=>$val){
			if($val['refund_real_money']==0 && $val['refund_status']!==5){$delivery_list[]=$val;}
		}
        $data['order_info'] = $order_info;
        $data['express_company_list'] = $express_company_list;
        $data['order_goods_list'] = $delivery_list;
        return $data;
    }
    ...
    public function orderDelivery()
    {
        ...
        if ($shipping_type == 1) {  //1是物流2是自提
			$order_express = new OrderExpress();
			$res = $order_express->delivey($order_id, $order_goods_id_array, $express_name, $shipping_type, $express_company_id, $express_no);
    ...
    {  //目前退货退款优先退余额，再提现，没有采取直接微信支付宝原路退回方式！
        $order_id = isset($_POST['order_id']) ? $_POST['order_id'] : $this->error("缺少必需参数order_id");
        $order_goods_id = isset($_POST['order_goods_id']) ? $_POST['order_goods_id'] : $this->error("缺少必需参数order_goods_id");
	...
	/*获取成本价格等*/
	public function ordergoodsCostprice(){
		$order_goods_id = isset($_POST['order_goods_id']) ? $_POST['order_goods_id'] : "";
		$onegoods=Db::table("ns_order_goods")->where("order_goods_id",$order_goods_id)->find();	
		return array('goods_name'=>$onegoods['goods_name'],'num'=>$onegoods['num'],'cost_price'=>$onegoods['cost_price']);
	}
    /**
     * 获取订单销售统计
     */
     ...
     * 虚拟商品点击确认完成订单
     */
    public function dummyComplete()
    {
		// 启动事务
		Db::startTrans();
		try{
			$order_id = request()->post('order_id');
			$status=Db::table("ns_order")->where("order_id",$order_id)->update(['order_status'=>4,'shipping_status'=>2,'finish_time'=>date("Y-m-d H::i:s", time())]);
			$ordergoods=Db::table("ns_order_goods")->where("order_id",$order_id)->select();
			$orderBusiness=new OrderBusiness();
			foreach($ordergoods as $k=>$v){
				$orderBusiness->shopSettlement($v['order_goods_id']);
				Db::table("ns_order_goods")->where("order_goods_id",$v['order_goods_id'])->update(['order_status'=>4,'shipping_status'=>2]);
			}
