...
class Order extends SimpleController
{
    ...
    public function orderPay()
    {
        ...
        if ($order_id != 0) {
            ...
            $url = 'http://' . $_SERVER['HTTP_HOST'] . \think\Request::instance()->root() . '/pcpay/getpayvalue?out_trade_no=' . $new_out_trade_no;
            header("Location: " . $url);
            exit();
        } else {
            // 待结算订单处理
            if ($out_trade_no != 0) {
                $url = 'http://' . $_SERVER['HTTP_HOST'] . \think\Request::instance()->root() . '/pcpay/getpayvalue?out_trade_no=' . $out_trade_no;
                exit();
            } else {
