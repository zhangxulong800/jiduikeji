...
class Goods extends BaseController
{
    ...
    public function goodsDetail()
    {
        ...
        $discount_id = isset($_GET['discount_id']) ? $_GET['discount_id'] : -1;
        if ($goods_id == 0) {
            $this->error("没有获取到商品信息");
        }
        $goods = new GoodsService();
        $goods_detail = $goods->getGoodsDetail($goods_id);
        $point = round(($goods_detail['market_price']-($goods_detail['cost_price']+$goods_detail['total_cost_price']))*0.4,2);
        $this->assign('point',$point);
		/*判断当前用户对该商品是否可以复购*/
		if (isset($this->uid)) {
			$buy_arr=$goods->alreadyByInfo($this->uid,$goods_id);
		} else {$buy_arr=array('num'=>0,'message'=>'未登陆不能购买！');}
		$this->assign('buy_arr',$buy_arr); 
        //把属性值相同的合并
...
//		}
        ...
        $goods_detail['price']=ceil($goods_detail['price']);
        $this->assign('goods_info',$goods_info['shipping_fee_name']);
        //var_dump($goods_info['shipping_fee_name']);
        $this->assign("goods_detail", $goods_detail);
        $this->assign("shopname", $this->shop_name);
        ...
        $this->assign("is_member_fav_goods", $is_member_fav_goods);
        $this->assign("type", $type);
        return view($this->style . 'Goods/excDetail');
    }
	/*首单包邮全免中介跳转页面*/
	public function firstorder()
    {
		$this->assign("skuid",$_GET['skuid']);
		return view($this->style . 'Goods/firstorder');
	}
     /**
     * 商品页面分享
     *
...
//        var_dump($goods_detail);
        ...
        return view($this->style . 'Goods/share');
    }
...
//        include_once('/data/pc_mob/cart.php');
//        $this->assign("list", $list);
//        $this->assign("countlist", count($cart_list));
...
//    }
    ...
    public function zeroAssemble()
    {
        ...
        $orderby = '';
        $goods = new GoodsService();
        $goods_list = $goods->getZeroGoodsList($page_index, $page_size, $condition, $orderby);
        return $goods_list;

        }else{
            return view($this->style . 'Goods/zeroAssemble');
        }
    }
	/**
     * 首单全免
    */
    ...
    {  
        if (request()->isAjax()) {
            $page_index = isset($_POST['page_index']) ? $_POST['page_index'] : '1'; // 倒排正排
            $page_size = isset($_POST['page_size']) ? $_POST['page_size'] : '0'; // 倒排正排
            $goods_arr = isset($_POST['goods_arr']) ? $_POST['goods_arr'] : '0';
            $condition = '';
            $orderby = '';
            $goods = new GoodsService();
            $goods_list = $goods->getMianGoodsList($page_index, $page_size, $condition, $orderby, $goods_arr);
            return $goods_list;
            ...
            return view($this->style . 'Goods/firstFree');
        }
    }
    /**
     * 商品列表
    ...
    public function goodsList()
    {
        if (request()->isAjax()) {
            ...
            $condition["ng.state"] = 1;
            $goods = new GoodsService();
            $goods_list = $goods->getGoodsList($page_index, $page_size, $condition, $orderby);
            return $goods_list;
        } else {
