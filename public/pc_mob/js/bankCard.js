...
function bankCardAttribution(bankCard,cmdIcon,id){
  var cardTypeMap = {
    DC: "储蓄卡",
    CC: "信用卡",
    SCC: "准贷记卡",
    ...
    for (var i = 0, len = bankcardList.length; i < len; i++) {
      ...
      for (var j = 0, jLen = patterns.length; j < jLen; j++) {
        ...
        if ((new RegExp(pattern.reg)).test(cardNo)) {
          	...
          	console.log(info)
          	if(cmdIcon != '1'){
          		html+='<a href="javascript:void(0)" id="mo" onclick="checkAccount('+id+',this)">'
	          		+'<div class="hasBank cf">'
								+'<div class="fl bankIcon">'
								+'<img src="'+cmdIcon+info.bankIcon+'"/>'
								+'</div>'
								+'<div class="fl bankCard">'
								+'<div class="bankName">'+info.bankName+'</div>'
								+'<div class="bankCord">'+'**** **** **** '+bankCard.substr(bankCard.length-4)+'</div>'
								+'</div>'
								+'</div>'
								+'<div class="delete" onclick="account_delete('+id+');">删除</div>'
								+'</a>'
	          	$('#hasBank').append(html);
          	}else{
          		$('#branch_bank_name').val(info.bankName);
          	}
          	if(id == true){
          		$('.bankName .name').html(info.bankName);
          		$('#bankIcon img').attr('src',cmdIcon+info.bankIcon)
          	}
      	return info;//返回银行卡结果
        }
      }
