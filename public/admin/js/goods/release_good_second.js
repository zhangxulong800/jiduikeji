...
function ValidateUserInput() {
	...
	if($("#shopGoodsType").val() == ''){
		$("#shopGoodsType").next("span").hide();
		$("#shopGoodsType").nextAll("span:last").show();
		$("#shopGoodsType").focus();
		return false;
	}else{
		$("#shopGoodsType").nextAll("span").hide();
	}
	// 副标题
...
//		return;
		if(id !=undefined){
            $.ajax({
                ...
                type : "post",
                async : false,
                data : { "product" : productViewObj , "is_qrcode" : $qrcode,"shop_id":id},
                dateType : "json",
                success : function(res) {
                    ...
                    if (res != null) {
                        ...
                        showMessage('success', "商品审核成功",ADMIN_MAIN +'/goods/goodslist');
                    } else {
                        showMessage('error', "商品审核失败",url);
                        flag = false;
...
function PackageProductInfo() {
	...
	// 12号 商品类目；
	if(!$('#txtProductMarketPrice')[0]){
        productViewObj.market_price = 0;
        productViewObj.cost_price_logistics = 0;
	}else{
        ...
        productViewObj.cost_price_logistics = $("#txtProductCostPriceL").val().replace(/^\s*/g, "").replace(/\s*$/g, "") == "" ? 0 : $("#txtProductCostPriceL").val().replace(/^\s*/g, "").replace(/\s*$/g,"");
	}
	...
	productViewObj.goods_type= $("#shopGoodsType").val();
